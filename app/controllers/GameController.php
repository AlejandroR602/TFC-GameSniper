...
class GameController {
    ...
    public function detail(string $slug): void {
        if (empty($slug)) { header('Location: ' . BASE_URL . '/search'); exit; }
    ...
    public function apiSearch(): void {
        ...
        if (empty($query)) { echo json_encode(['error' => 'Parámetro q requerido']); return; }

        $url = RAWG_BASE_URL . '/games?' . http_build_query([
            'key' => RAWG_API_KEY, 'search' => $query,
    ...
    public function apiPrices(): void {
        ...
        if (empty($title)) { echo json_encode(['deals' => []]); return; }
        ...
        if (empty($search[0]['id'])) { echo json_encode(['deals' => []]); return; }
    ...
    public function apiIgdb(): void {
        $title = trim($_GET['title'] ?? '');
        if (empty($title)) { echo json_encode(['error' => 'title requerido']); return; }

        $token = $this->getIgdbToken();
        if (!$token) { echo json_encode(['error' => 'No se pudo obtener token IGDB. Comprueba tus credenciales Twitch.']); return; }
        ...
        if (empty($data[0])) { echo json_encode([]); return; }
